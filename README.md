...
The $n^{-1/2}$ rate is dimension-free. It is the reason Monte Carlo is the tool of choice for path-dependent and high-dimensional payoffs but it comes at a cost: a tenfold accuracy gain costs a hundredfold more paths. This is the reasoning for using variance reduction.

**Antithetic variates.** Each normal draw $Z$ is paired with $-Z$ and the two payoffs are averaged. The pair mean has variance $\tfrac{1}{2}\sigma^2(1+\rho_a)$ where $\rho_a=\mathrm{corr}(\Phi(Z),\Phi(-Z))$, so for the same total path count the SE ratio against plain MC is $\sqrt{1+\rho_a}$. The method is helpful only when the payoff is monotone enough in $Z$ to make $\rho_a$ negative.
...
Unless noted, the test contract is $S_0 = 100$, $K = 100$, $r = 5\%$, $\sigma = 20\%$, $T = 1$. All figures and numbers below are regenerated by `python3 make_assets.py` and stored in `assets/results.json`. All acceptance bands are ±3SE unless stated otherwise.
...
**Sanity check on the sampler itself:** the simulated mean of $S_T$ is 105.1869 against the exact value $S_0e^{rT} = 105.1271$, which is inside the 3-SE band, and the terminal histogram overlays the exact lognormal density as expected.

![GBM terminal distribution](assets/gbm_distribution.png)

### 2. Put–call parity (model-free)

Parity is worth testing because it does not depend on Black–Scholes being right as it is an arbitrage identity, so a failure is a bug in the pricer, not a modelling disagreement. The call and put are priced on independent draws, so the SE of C-P is the two SEs combined in quadrature.
...
This produces a residual of 0.52 SE which sits well within the 3SE band.

### 3. Variance reduction

![Variance reduction](assets/variance_reduction.png)

All three estimators come out with similar slopes as expected; **−0.497** (plain), **−0.494** (antithetic), **−0.498** (control variate). Variance reduction does not improve the $n^{-1/2}$ rate, it only shifts the intercept down. People usually misinterpret it as "a better convergence rate", and the parallel slopes disprove it.
...
**Note:** ρ_a is the correlation within an antithetic pair; ρ is payoff–control

The control variate is worth roughly a **6.8× saving in paths** for equal accuracy, at the cost of an extra vectorized payoff evaluation. The measured ratios match the predicted ones to within 0.003, so the reduction is the size theory predicts.

### 4. Asian options and monitoring bias

![Asian discretisation bias](assets/asian_bias.png)

The geometric Asian is the useful test case because it has a closed form in both flavours. Kemna–Vorst prices the *continuously* averaged contract at **5.5468**, but the simulation averages over a finite set of dates. Comparing the two directly would conflate two separate errors, so the check separates them.

Because $\log S$ is Gaussian, the discretely monitored geometric average is itself lognormal, and `kmv_call_discrete` prices it exactly, with $\mathrm{Var}[\log A_m] = \sigma^2 T (2m{+}1)/(6(m{+}1)) \to \sigma^2T/3$ as $m \to \infty$. This shows two things:

- **The simulation is correct.** Across all nine grids from $m=2$ to $m=512$, the MC price never strays from the exact discrete price by more than **1.36 SE**.
- **The remaining gap is a contract difference, not a bug.** The exact discretisation bias decays with a log-log slope of **−0.94**, i.e. $O(1/m)$, it halves every time the monitoring frequency doubles, from −0.443 at $m=2$ to −0.0025 at $m=512$.
...
Three checks were carried out.

**In + out = vanilla.** A knock-in and a knock-out with the same barrier partition every path between them, so their prices must sum to the vanilla. As they were priced on identical seeds, the residual is at most **1.8 × 10⁻¹⁵** across all eleven barrier levels.

**Limiting cases.** An up-and-out call with $B = 10^6$ (unreachable) prices at 10.4946 ± 0.0330 against the vanilla Black–Scholes 10.4506, a gap of 1.33 SE, correctly reducing to the vanilla. An up-and-out call with the barrier at the spot prices at exactly 0.0000.

**The control-variate regime.** The plot shows SE(control variate) / SE(plain) as the barrier moves out, where lower is better. The two legs cross between $B = 150$ and $B = 160$, and the shape proves the theory which shows that the vanilla call is an effective control only for whichever leg still *resembles* a vanilla. For a low barrier, knock-in is almost certain, the in-option is nearly the vanilla, and the control cuts the SE by a factor of 66 (ratio 0.015 at $B=105$), while the out-option is a thin tail with almost no vanilla content and the control does nothing (ratio 1.00). On the opposite end, this reverses: at $B=220$ the out-option is nearly the vanilla (ratio 0.083) and the in-option is the residual (ratio 0.997). A ratio near 1.0 is not a failure of the implementation; it is the correct report that no variance reduction is available from this control in that regime.
...
All three pass the 3 SE band with Vega sitting at 2.03 SE, the weakest of the three, though still inside the band. Gamma is the interesting case: the second difference of a payoff with a kink is dominated by the paths that finish near the strike, and it still lands within 1 SE. The left panel extends the delta check across spot from 70 to 130, where the MC points track the analytic curve through the whole moneyness range rather than only at the money.

**The CRN A/B test** is the right panel. Holding everything else fixed and changing only whether the two legs share a seed:
...
Both are unbiased but only one is usable. Recovering the CRN precision by brute force would take roughly 1,300× as many paths.
